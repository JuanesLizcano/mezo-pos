...
export default function TicketPDF({ orden, negocio }) {
  const {
    ...
    total = 0, metodo, recibido, cambio, cajero, fecha,
  } = orden;
  ...
  return (
    <Document>
      <Page size={{ width: ANCHO_RECIBO, height: 'auto' }} style={S.page} wrap={false}>
        ...
        {/* Logo y negocio */}
        <View style={S.header}>
          <Text style={S.logoTxt}>mezo</Text>
          {negocio?.nombre && <Text style={S.negocio}>{negocio.nombre}</Text>}
          {negocio?.ciudad && <Text style={S.subtitulo}>{negocio.ciudad}</Text>}
          {negocio?.horario && <Text style={S.subtitulo}>{negocio.horario}</Text>}
        </View>

        {/* Info de orden */}
        <View style={S.fila}>
          <Text style={S.labelTxt}>Orden</Text>
          <Text style={S.valorTxt}>#{ordenNum}</Text>
        </View>
        <View style={S.fila}>
        ...
        {lineas.map((l, i) => (
          <View key={i} style={S.fila}>
            <Text style={S.itemQty}>{l.cantidad}×</Text>
            <Text style={S.itemNombre}>{l.nombre}</Text>
        ...
        {/* IVA incluido */}
        <Text style={S.ivaNote}>
          IVA incluido (8%): {formatCOPPDF(ivaIncluido)}
        </Text>
        ...
        {/* Footer */}
        <View style={S.footer}>
          <Text style={S.footerTxt}>¡Gracias por tu visita!</Text>
